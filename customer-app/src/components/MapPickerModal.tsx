...
async function reverseGeocode(
  ...
  const a = d.address || {};
  const street = [
    a.road || a.pedestrian || a.street || a.footway,
    a.house_number,
  ]
    .filter(Boolean)
    .join(" ");
...
export default function MapPickerModal({
  ...
  return createPortal(
    ...
    >
      {/* Header */}
      <div className="relative px-4 pt-4 pb-3 border-b border-border-light flex items-center gap-3 shrink-0 bg-white">
        <motion.button
          whileTap={{ scale: 0.88 }}
          onClick={onClose}
