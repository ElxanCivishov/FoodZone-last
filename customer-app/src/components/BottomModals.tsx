...
import {
  X, Wifi, Copy, Clock, Sun, Moon, Monitor,
  Star, Send, CheckCircle2, ChevronRight, User, ChevronLeft,
  Bell, CreditCard, Droplets, ScrollText, Sparkles, HelpCircle,
} from 'lucide-react';
      ...
      {activeModal && (
        <>
          ...
          >
            ...
            {activeModal === 'wifi'       && <WifiSheet     close={closeModal} addToast={addToast} />}
            {activeModal === 'language'   && <LangSheet     close={closeModal} />}
            {activeModal === 'hours'      && <HoursSheet    close={closeModal} />}
            {activeModal === 'colorMode'  && <ColorSheet    close={closeModal} addToast={addToast} />}
            {activeModal === 'feedback'   && <FeedbackSheet close={closeModal} addToast={addToast} />}
            {activeModal === 'waiterCall' && <WaiterCallSheet close={closeModal} addToast={addToast} t={t} />}
          </motion.div>
        </>
      )}
    </AnimatePresence>
...
function ModalHeader({ title, close }: { title: string; close: () => void }) {
  return (
    <div className="flex items-center justify-between px-5 pb-4 border-b border-border-light">
      <h3 className="font-outfit text-lg font-bold text-text-primary">{title}</h3>
      <motion.button
        whileTap={{ scale: 0.88 }}
        onClick={close}
...
/* ─── WiFi ─── */
function WifiSheet({ close, addToast }: { close: () => void; addToast: (m: string, t?: 'success' | 'error' | 'info') => void }) {
  return (
  ...
  return (
    <div className="w-full h-full relative flex items-center justify-center">
      <svg viewBox="0 0 100 100" className="w-full h-full">
        ...
        <text x="50" y="54" textAnchor="middle" fill="white" fontSize="8" fontWeight="bold" fontFamily="sans-serif">
...
function WifiNetworkPicker({ addToast, compact = false }: { addToast: (m: string, t?: 'success' | 'error' | 'info') => void; compact?: boolean }) {
  ...
  const copyField = (field: 'name' | 'pass') => {
    ...
    addToast(field === 'name' ? 'Şəbəkə adı kopyalandı!' : 'Şifrə kopyalandı!', 'success');
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-1">
        <p className="text-[13px] font-semibold text-text-secondary">{net.label}</p>
        <div className="flex gap-1.5">
          {RESTAURANT_INFO.wifiNetworks.map((_, idx) => (
            <motion.button
              key={idx}
              whileTap={{ scale: 0.88 }}
              onClick={() => { setActiveIdx(idx); setCopiedField(null); }}
              className={`w-7 h-7 rounded-full text-[12px] font-bold transition-all flex items-center justify-center ${
                activeIdx === idx ? 'bg-primary text-white shadow-primary-glow' : 'bg-surface-elevated text-text-secondary border border-border-light'
              }`}
            >
              {idx + 1}
            </motion.button>
        ...
        >
          ...
          {!compact && (
            <div className="flex flex-col items-center mb-5">
              ...
              <p className="text-text-tertiary text-[12px]">Scan QR or manually copy the password</p>
            </div>
          )}

          {/* Field rows */}
          <div className="space-y-2">
            <WifiFieldRow label="Name" value={net.ssid} copied={copiedField === 'name'} onCopy={() => copyField('name')} />
            <WifiFieldRow label="Password" value={net.pass} copied={copiedField === 'pass'} onCopy={() => copyField('pass')} />
          </div>
        </motion.div>
      </AnimatePresence>
    </div>
...
function WifiFieldRow({
  ...
  return (
    <div className="bg-surface-elevated border border-border-light rounded-2xl px-4 py-3.5 flex items-center justify-between">
      <div>
        <p className="text-text-tertiary text-[11px] font-semibold mb-0.5">{label}</p>
        ...
        {LANGUAGES.map((lang) => {
          ...
          return (
            ...
            >
              ...
              <span className={`flex-1 text-[15px] font-semibold text-left ${active ? 'text-primary' : 'text-text-primary'}`}>
                {lang.label}
              </span>
          ...
          return (
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span className={`text-[14px] font-medium ${isToday ? 'text-primary' : 'text-text-secondary'}`}>
                  {h.day}
                  {isToday && (
                    <span className="ml-1.5 text-[11px] font-bold bg-primary text-white rounded-full px-2 py-0.5">Bu gün</span>
                  )}
                </span>
              </div>
              <span className={`text-[14px] font-semibold ${isToday ? 'text-primary' : 'text-text-primary'}`}>{h.time}</span>
            </div>
          );
        })}
      </div>
...
/* ─── Color Mode ─── */
function ColorSheet({ close, addToast }: { close: () => void; addToast: (m: string, t?: 'success' | 'error' | 'info') => void }) {
      ...
      <div className="mt-4 flex gap-3">
        {COLOR_MODES.map((m) => {
          ...
          return (
            ...
            >
              <Icon size={24} className={active ? 'text-primary' : 'text-text-secondary'} />
              <span className={`text-[13px] font-semibold ${active ? 'text-primary' : 'text-text-secondary'}`}>{m.label}</span>
            </motion.button>
          );
        })}
      </div>
...
function StarRow({ label, value, onChange }: { label: string; value: number; onChange: (v: number) => void }) {
  ...
  return (
    <div>
      <p className="text-text-primary text-[15px] font-semibold mb-3">{label}</p>
          ...
          >
            <Star
              ...
              className={`transition-colors ${
                s <= (hovered || value) ? 'text-warning fill-warning' : 'text-border fill-surface-elevated'
              }`}
            />
          </motion.button>
        ))}
...
function FeedbackContent({ close, addToast }: { close: () => void; addToast: (m: string, t?: 'success' | 'error' | 'info') => void }) {
  ...
  if (sent) {
    return (
      <div className="flex flex-col items-center gap-4 py-12 px-5">
        ...
        <p className="font-outfit text-xl font-bold text-text-primary text-center">Təşəkkür edirik!</p>
        <p className="text-text-secondary text-[13px] text-center">Rəyiniz qeydə alındı</p>
      </div>
    );
  }
    ...
    return (
      <div className="pb-8">
        <div className="flex items-center gap-3 px-5 py-3 border-b border-border-light">
          ...
          <h3 className="font-outfit text-[15px] font-bold text-text-primary">Ofisiant seçin</h3>
        </div>
        <div className="px-5 mt-4 space-y-2">
          {WAITERS.map((w) => {
            ...
            return (
              <motion.button
                key={w.id}
                whileTap={{ scale: 0.97 }}
                onClick={() => { setSelectedWaiter(w); setStep('main'); }}
              ...
              >
                ...
                <span className={`flex-1 text-[15px] font-medium text-left ${active ? 'text-primary' : 'text-text-primary'}`}>{w.name}</span>
          ...
          <motion.button
            whileTap={{ scale: 0.97 }}
            onClick={() => { setSelectedWaiter(null); setStep('main'); }}
          ...
          >
            ...
            <span className="text-[14px] font-medium text-text-secondary text-left">Ofisiant seçmə</span>
          </motion.button>
        </div>
      </div>
    );
    ...
    <div className="px-5 pt-4 pb-8 space-y-5">
      ...
      >
        ...
        <div className="flex-1 text-left">
          ...
          <p className="text-text-tertiary text-[12px] mt-0.5">Xüsusi rəy üçün</p>
        </div>
      ...
      <StarRow label="Xidmət" value={serviceRating} onChange={setServiceRating} />
...
function FeedbackSheet({ close, addToast }: { close: () => void; addToast: (m: string, t?: 'success' | 'error' | 'info') => void }) {
  return (
    <div className="pb-2">
...
function WaiterCallSheet({
  close, addToast, t,
}: { close: () => void; addToast: (m: string, t?: 'success' | 'error' | 'info') => void; t: Translations }) {
  ...
  const send = () => {
    ...
    setTimeout(() => {
      ...
      setStatus('sent');
      addToast(t.waiter.sent, 'success');
    }, 1200);
  };
    ...
    return (
      <div className="px-5 pb-10 pt-2">
        <div className="flex justify-end mb-2">
          <motion.button whileTap={{ scale: 0.88 }} onClick={close}
            className="w-8 h-8 rounded-full bg-surface-elevated flex items-center justify-center">
            <X size={16} className="text-text-secondary" />
          </motion.button>
        </div>
        <div className="flex flex-col items-center gap-3 py-4">
          ...
          <h3 className="font-outfit text-[20px] font-bold text-text-primary">{t.waiter.sent}</h3>
          <p className="text-text-secondary text-[13px] text-center">{t.waiter.note}</p>
    ...
    <div className="px-5 pb-8 pt-2">
      ...
      <div className="flex items-center justify-between mt-4 mb-3">
        <p className="text-text-secondary text-[13px]">
          {t.waiter.subtitle}
          {tableNumber > 0 && (
            <span className="ml-1.5 text-text-tertiary">· Masa #{tableNumber}</span>
          )}
        </p>
        {requests.length > 0 && (
          <motion.button
        ...
        {REQUEST_TYPES.map(({ type, icon: Icon }) => {
          ...
          return (
            ...
            >
              ...
              />
              <span className={`text-[12px] font-semibold ${active ? 'text-primary' : 'text-text-secondary'}`}>
                {t.waiter.types[type]}
              </span>
            </motion.button>
          );
