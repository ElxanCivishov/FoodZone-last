...
import {
  ChevronLeft, Star, Check, Minus, Plus, ShoppingBag, Heart,
} from 'lucide-react';
...
export default function ProductDetail() {
  const { productModalOpen, selectedProduct, closeProductModal, openCartDrawer, addToast } =
    useUIStore();
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar">
              ...
              <div className="relative mx-4 h-[260px] rounded-2xl overflow-hidden">
                ...
                >
                  <Heart
                    size={16}
                    className={liked ? 'text-coral fill-coral' : 'text-text-tertiary'}
                  />
                </motion.button>
              ...
              <div className="px-5 pt-5 pb-6">
                ...
                <p className="text-[13px] text-text-secondary mt-1">({selectedProduct.reviews} rəy)</p>
                <p className="text-[14px] text-text-secondary leading-relaxed mt-3">{selectedProduct.desc}</p>

                {/* Size */}
                <h3 className="font-outfit text-[15px] font-bold text-text-primary mt-6 mb-3">Ölçü</h3>
                  ...
                  {sizeOptions.map((size) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-[13px] font-semibold text-text-primary">{size.label}</span>
                      </div>
                      <span className="text-[13px] text-text-secondary">+{size.priceModifier} AZN</span>
                    </motion.button>
                  ))}
                </div>

                {/* Extras */}
                <h3 className="font-outfit text-[15px] font-bold text-text-primary mt-6 mb-3">Əlavələr</h3>
                <div className="space-y-2">
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-[13px] font-semibold text-text-primary">{extra.label}</span>
                        </div>
                        <span className="text-[13px] text-text-secondary">+{extra.price} AZN</span>
                      </motion.button>
                    );
                  })}
                </div>
                ...
                <motion.button
                  ...
                  style={added ? {} : { background: 'linear-gradient(135deg, #00c2e8, #00c2a8)' }}
                >
                  <AnimatePresence mode="wait">
                    {added ? (
                      <motion.span key="ok" initial={{ opacity: 0, scale: 0.5 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="flex items-center gap-2">
                      ...
                      <motion.span key="add" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex items-center gap-2">
                        <ShoppingBag size={17} /> Səbətə əlavə et — {total.toFixed(2)} AZN
                      </motion.span>
                    )}
                  </AnimatePresence>
                </motion.button>
